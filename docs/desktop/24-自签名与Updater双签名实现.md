...
## 5. 本机验证证据（2026-08-31，最新复测）

最新安装包从源码提交 `bc373242ff09f0d922fcec7481f182a0e7871174` 构建；该提交包含完整本地主体功能和发布就绪审计。旧体积记录已由本次最终字节替代。

| 制品 | 字节数 | Authenticode | 内层主程序/sidecar | Updater Ed25519 |
| --- | ---: | --- | --- | --- |
| `时奕教务排课_0.1.0_x64-setup.exe` | 75,867,056 | 指纹匹配，DigiCert 时间戳 | 均通过 | `.sig` 432 字节，通过 |
| `时奕教务排课_0.1.0_x64_zh-CN.msi` | 77,524,992 | 指纹匹配，DigiCert 时间戳 | 均通过 | `.sig` 432 字节，通过 |
